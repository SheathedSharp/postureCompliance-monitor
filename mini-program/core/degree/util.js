...
/**
 * @description 对16进制数据作矫正，返回十进制数
 * @param {string} str - 16进制的字符串格式
 * @returns {number} ret - 10进制的数字格式
 * @version 1.4.3
...
// }
//编写命令帧
const stringifyReq = function (command, data) {
  switch (command) {
    case cl.TRANSMIT_DATA_REQ:
      if (data.start === undefined || data.length === undefined) {
        throw "data.start or data.length is not defined"
      }
      console.log("AA" + cl.TRANSMIT_DATA_REQ + inCorrectData(data.start, 8) + inCorrectData(data.length, 8) + "0000")
      return calculateCheckByte("AA" + cl.TRANSMIT_DATA_REQ + inCorrectData(data.start, 8) + inCorrectData(data.length, 8) + "0000")
      break;
    case cl.DELETE_DATA_REQ:
      break;
    case cl.GET_DEGREE_REQ:
      break;
    case cl.CHANGE_STATE_REQ:
      break;
    case cl.CANCLE_ALARM_REQ:
      break;
    case cl.SET_WARN_DATA_REQ:
      console.log("AA" + cl.SET_WARN_DATA_REQ + inCorrectData(data.degree, 4) + inCorrectData(data.degree, 4) + inCorrectData(data.degree, 4) + "0000")
      return calculateCheckByte("AA" + cl.SET_WARN_DATA_REQ + inCorrectData(data.degree, 4) + inCorrectData(20, 4) + inCorrectData(20, 4) + "0000")
      break;
    case cl.GET_RECORD_NUM_REQ:
      break;
  }
}
//根据命令帧，验证响应帧
const validRes = function (req, res) {
  if (req[3] != res[3] || parseInt(res[res.length - 2], 16) < 10 || calculate(res.slice(0, res.length - 4)) != res.slice(res.length - 4, res.length - 2)) {
    return 0
  }
  return 1
}
